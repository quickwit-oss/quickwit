...
use crate::service::cluster_service::ClusterServiceImpl;
use crate::service::search_service::SearchServiceImpl;

/// Start gRPC service given a gRPC address and a cluster.
pub async fn start_service(grpc_addr: SocketAddr, cluster: Cluster) -> anyhow::Result<()> {
    let cluster = Arc::new(cluster);

    info!(grpc_addr=?grpc_addr, "Start gRPC service.");
    let cluster_service = ClusterServiceImpl::new(cluster.clone());
    let search_service = SearchServiceImpl::new(cluster.clone()).await?;
    Server::builder()
