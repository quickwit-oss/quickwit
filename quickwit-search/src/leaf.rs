...
use quickwit_proto::{
    SplitAndFooterOffsets, LeafSearchResult, SearchRequest, SplitSearchError,
};
...
pub(crate) async fn open_index(index_storage: Arc<dyn Storage>, split_and_footer_offsets: &SplitAndFooterOffsets) -> anyhow::Result<Index> {
    let split_file = PathBuf::from(format!("{}.split", split_and_footer_offsets.split_id));
    let mut footer_data = index_storage
        .get_slice(
                &split_file,
                split_and_footer_offsets.split_footer_start as usize..split_and_footer_offsets.split_footer_end as usize,
        )
        .await
        .with_context(|| {
            format!(
    ...
    let hotcache_bytes =
        footer_data.split_off(footer_data.len() - hotcache_num_bytes);
