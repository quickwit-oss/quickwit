...
const DEFAULT_INGEST_ROUTER_BUFFER_SIZE: u64 = if cfg!(test) { 8 } else { 256 }; // 256 MiB
...
pub(crate) fn get_ingest_router_buffer_size() -> ByteSize {
    env::var(INGEST_ROUTER_BUFFER_SIZE_ENV_KEY)
        ...
        .and_then(|buffer_size_bytes_str| {
            if let Ok(buffer_size_bytes) = buffer_size_bytes_str.parse::<u64>() {
                let buffer_size = ByteSize::b(buffer_size_bytes);
        ...
        .unwrap_or(ByteSize::mib(DEFAULT_INGEST_ROUTER_BUFFER_SIZE))
}
