...
use quickwit_proto::ingest::router::{IngestFailureReason, IngestRequestV2, IngestResponseV2, IngestRouterService};
...
fn update_ingest_metrics(ingest_result: &IngestV2Result<IngestResponseV2>, num_subrequests: usize) {
    ...
    let ingest_results_metrics: &IngestResultMetrics = &crate::ingest_v2::metrics::INGEST_V2_METRICS.ingest_results;
    match ingest_result {
        Ok(ingest_response) => {
            ingest_results_metrics.success.inc_by(ingest_response.successes.len() as u64);
            for ingest_failure in &ingest_response.failures {
                match ingest_failure.reason() {
                    ...
                    IngestFailureReason::IndexNotFound => ingest_results_metrics.index_not_found.inc(),
                    IngestFailureReason::SourceNotFound => ingest_results_metrics.source_not_found.inc(),
                    IngestFailureReason::Internal => ingest_results_metrics.internal.inc(),
                    IngestFailureReason::NoShardsAvailable => ingest_results_metrics.no_shards_available.inc(),
                    IngestFailureReason::ShardRateLimited => ingest_results_metrics.shard_rate_limited.inc(),
                    ...
                    IngestFailureReason::RouterLoadShedding => ingest_results_metrics.router_load_shedding.inc(),
        ...
        Err(ingest_error) => {
            match ingest_error {
                IngestV2Error::TooManyRequests(rate_limiting_cause) => {
                    match rate_limiting_cause {
                        RateLimitingCause::RouterLoadShedding => {
                            ingest_results_metrics.router_load_shedding.inc_by(num_subrequests);
                        },
                        ...
                        RateLimitingCause::CircuitBreaker => {
                            ingest_results_metrics.circuit_breaker.inc_by(num_subrequests);
                        },
                        RateLimitingCause::ShardRateLimiting => {
                            ingest_results_metrics.shard_rate_limited.inc_by(num_subrequests);
                        },
                ...
                IngestV2Error::Timeout(_) => {
                    ingest_results_metrics.router_timeout.inc_by(num_subrequests);
                }
                IngestV2Error::ShardNotFound { .. }  => {
                    ingest_results_metrics.shard_not_found.inc_by(num_subrequests);
                }
