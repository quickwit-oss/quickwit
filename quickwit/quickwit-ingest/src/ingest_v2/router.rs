...
use super::broadcast::{IngesterCapacityUpdate, LocalShardsUpdate};
...
impl IngestRouter {
    ...
    pub fn subscribe(&self) {
        ...
        self.event_broker
            ...
            .forever();
        self.event_broker
            .subscribe::<IngesterCapacityUpdate>(weak_router_state)
            .forever();
    }
...
#[async_trait]
impl EventSubscriber<IngesterCapacityUpdate> for WeakRouterState {
    async fn handle_event(&mut self, update: IngesterCapacityUpdate) {
        let Some(state) = self.0.upgrade() else {
            return;
        };
        let mut state_guard = state.lock().await;
