...
impl IngestServiceClient {
    ...
    ) -> ingest_service_grpc_server::IngestServiceGrpcServer<IngestServiceGrpcServerAdapter> {
    ...
    pub fn from_channel(
        ...
        let mut client = ingest_service_grpc_client::IngestServiceGrpcClient::new(channel)
            .max_decoding_message_size(max_message_size.0 as usize)
            .max_encoding_message_size(max_message_size.0 as usize);
        ...
        let adapter = IngestServiceGrpcClientAdapter::new(client, connection_keys_watcher);
        Self::new(adapter)
    }
    pub fn from_balance_channel(
        ...
        let mut client = ingest_service_grpc_client::IngestServiceGrpcClient::new(balance_channel)
            .max_decoding_message_size(max_message_size.0 as usize)
            .max_encoding_message_size(max_message_size.0 as usize);
        if let Some(compression_encoding) = compression_encoding_opt {
            client = client
...
pub mod mock_ingest_service {
    ...
    impl IngestService for MockIngestServiceWrapper {
        ...
        async fn fetch(&self, request: super::FetchRequest) -> crate::Result<super::FetchResponse> {
            self.inner.lock().await.fetch(request).await
        }
        async fn tail(&self, request: super::TailRequest) -> crate::Result<super::FetchResponse> {
...
type IngestLayer = quickwit_common::tower::BoxLayer<
    quickwit_common::tower::BoxService<IngestRequest, IngestResponse, crate::IngestServiceError>,
    IngestRequest,
    IngestResponse,
    crate::IngestServiceError,
>;
...
impl IngestServiceTowerLayerStack {
    ...
    where
        ...
        <L as tower::Layer<
            quickwit_common::tower::BoxService<
                ...
                crate::IngestServiceError,
            >,
        >>::Service: tower::Service<TailRequest, Response = FetchResponse, Error = crate::IngestServiceError>
            + Clone
            + Send
            + Sync
            + 'static,
    ...
    where
        L: tower::Layer<
            ...
            + Sync
            + 'static,
        L::Service: tower::Service<TailRequest, Response = FetchResponse, Error = crate::IngestServiceError>
            + Clone
            + Send
            + Sync
            + 'static,
    ...
    {
        let inner_client =
            InnerIngestServiceClient(std::sync::Arc::new(IngestServiceMailbox::new(mailbox)));
        self.build_from_inner_client(inner_client)
    }
    #[cfg(any(test, feature = "testsuite"))]
    ...
    fn build_from_inner_client(
        ...
        let fetch_svc = self.fetch_layers.into_iter().rev().fold(
            quickwit_common::tower::BoxService::new(inner_client.clone()),
            |svc, layer| layer.layer(svc),
        );
        let tail_svc = self.tail_layers.into_iter().rev().fold(
            quickwit_common::tower::BoxService::new(inner_client.clone()),
            |svc, layer| layer.layer(svc),
        );
    ...
    T::Future: Send,
{
    async fn ingest(&self, request: IngestRequest) -> crate::Result<IngestResponse> {
        self.inner
            ...
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, IngestRequest::rpc_name())
            })
    }
    async fn fetch(&self, request: FetchRequest) -> crate::Result<FetchResponse> {
        self.inner
            ...
            .map(|response| response.into_inner())
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, FetchRequest::rpc_name())
            })
    }
    async fn tail(&self, request: TailRequest) -> crate::Result<FetchResponse> {
        self.inner
            ...
            .map(|response| response.into_inner())
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, TailRequest::rpc_name())
            })
    }
}
#[derive(Debug)]
