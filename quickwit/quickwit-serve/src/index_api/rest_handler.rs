...
mod tests {
    ...
    async fn test_create_file_source_returns_405() {
        ...
        let index_management_handler =
            super::index_management_handlers(Arc::new(index_service), Arc::new(quickwit_config))
                .recover(recover_fn);
        let source_config_body = r#"{"version": "0.7", "source_id": "file-source", "source_type": "file", "params": {"filepath": "FILEPATH"}}"#;
        let resp = warp::test::request()
            .path("/indexes/hdfs-logs/sources")
            .method("POST")
            .body(source_config_body)
    ...
    async fn test_create_source_with_bad_config() {
        ...
        }
        {
            ...
            let resp = warp::test::request()
                ...
                .method("POST")
                .json(&true)
                .body(r#"{"version": "0.7", "source_id": "pulsar-source", "desired_num_pipelines": 2, "source_type": "pulsar", "params": {"topics": ["my-topic"], "address": "pulsar://localhost:6650" }}"#)
                .reply(&index_management_handler)
                .await;
            assert_eq!(resp.status(), 400);
            let body = from_utf8_lossy(resp.body());
