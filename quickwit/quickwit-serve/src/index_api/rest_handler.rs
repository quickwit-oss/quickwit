...
mod tests {
    ...
    async fn test_create_index_with_overwrite() {
            ...
            super::index_management_handlers(index_service, Arc::new(node_config));
        {
            let resp = warp::test::request()
                ...
                .method("POST")
                .json(&true)
                .body(r#"{"version": "0.8", "index_id": "hdfs-logs", "doc_mapping": {"field_mappings":[{"name": "timestamp", "type": "i64", "fast": true, "indexed": true}]}}"#)
                .reply(&index_management_handler)
                .await;
            assert_eq!(resp.status(), 200);
        }
    ...
    async fn test_create_delete_index_and_source() {
        ...
        // Create source.
        let source_config_body = r#"{"version": "0.8", "source_id": "vec-source", "source_type": "vec", "params": {"docs": [], "batch_num_docs": 10}}"#;
        let resp = warp::test::request()
    ...
    async fn test_create_index_with_bad_config() -> anyhow::Result<()> {
        ...
        let resp = warp::test::request()
            ...
            .json(&true)
            .body(
                r#"{"version": "0.8", "index_id": "hdfs-log", "doc_mapping":
    ...
    async fn test_update_index() {
            ...
            super::index_management_handlers(index_service, Arc::new(node_config));
        {
            let resp = warp::test::request()
                ...
                .method("POST")
                .json(&true)
                .body(r#"{"version": "0.8", "index_id": "hdfs-logs", "doc_mapping": {"field_mappings":[{"name": "timestamp", "type": "i64", "fast": true, "indexed": true}]},"search_settings":{"default_search_fields":["body"]}}"#)
                .reply(&index_management_handler)
                .await;
            assert_eq!(resp.status(), 200);
            let resp_json: serde_json::Value = serde_json::from_slice(resp.body()).unwrap();
        ...
        }
        {
            let resp = warp::test::request()
                ...
                .method("PUT")
                .json(&true)
                .body(r#"{"version": "0.8", "index_id": "hdfs-logs", "doc_mapping": {"field_mappings":[{"name": "timestamp", "type": "i64", "fast": true, "indexed": true}]},"search_settings":{"default_search_fields":["severity_text", "body"]}}"#)
                .reply(&index_management_handler)
                .await;
            assert_eq!(resp.status(), 200);
            let resp_json: serde_json::Value = serde_json::from_slice(resp.body()).unwrap();
        ...
        // test with index_uri at the root of a bucket
        {
            let resp = warp::test::request()
                ...
                .method("POST")
                .json(&true)
                .body(r#"{"version": "0.8", "index_id": "hdfs-logs2", "index_uri": "s3://my-bucket", "doc_mapping": {"field_mappings":[{"name": "timestamp", "type": "i64", "fast": true, "indexed": true}]},"search_settings":{"default_search_fields":["body"]}}"#)
                .reply(&index_management_handler)
                .await;
            let body = std::str::from_utf8(resp.body()).unwrap();
        ...
        }
        {
            let resp = warp::test::request()
                ...
                .method("PUT")
                .json(&true)
                .body(r#"{"version": "0.8", "index_id": "hdfs-logs2", "index_uri": "s3://my-bucket", "doc_mapping": {"field_mappings":[{"name": "timestamp", "type": "i64", "fast": true, "indexed": true}]},"search_settings":{"default_search_fields":["severity_text", "body"]}}"#)
                .reply(&index_management_handler)
                .await;
            let body = std::str::from_utf8(resp.body()).unwrap();
            assert_eq!(resp.status(), 200, "{body}",);
