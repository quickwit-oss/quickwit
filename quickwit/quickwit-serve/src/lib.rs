...
use quickwit_cluster::{
    Cluster, ClusterChange, ClusterChangeStream, ClusterNode, start_cluster_service,
};
...
use quickwit_ingest::{
    GetMemoryCapacity, IngestListenerHandle, IngestRequest, IngestRouter, IngestServiceClient,
    Ingester, IngesterClient, IngesterPool, IngesterStatusUpdate, LocalShardsUpdate,
    get_idle_shard_timeout, setup_ingest_listener, start_ingest_api_service,
        ...
        Box::pin(async move {
            match cluster_change {
                ClusterChange::Add(node) if node.is_indexer() => {
                    ...
                    let ingester_client = build_ingester_client(
                        &node,
                        ingester_opt_clone_clone,
                        max_message_size,
                        grpc_compression_encoding_opt,
                    );
                    Some(Change::Insert(node_id, ingester_client))
                }
                ClusterChange::Update(node) if node.is_indexer() => {
                    let node_id: NodeId = node.node_id().into();
                    let ingester_client = build_ingester_client(
                        &node,
                        ingester_opt_clone_clone,
