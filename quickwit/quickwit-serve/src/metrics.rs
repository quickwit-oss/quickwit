...
use quickwit_common::metrics::{
    new_counter_vec, new_gauge, new_histogram_vec, HistogramVec, IntCounterVec, IntGauge,
};

pub struct RestMetrics {
    pub http_requests_total: IntCounterVec<2>,
    pub request_duration_secs: HistogramVec<2>,
    pub ingest_ongoing_requests: IntGauge,
    pub ingest_pending_requests: IntGauge,
}
...
impl Default for RestMetrics {
    fn default() -> Self {
        RestMetrics {
            ...
            ingest_ongoing_requests: new_gauge(
                "ingest_ongoing_requests",
                "Number of ongoing ingest requests.",
                "",
                &[],
            ),
            ingest_pending_requests: new_gauge(
