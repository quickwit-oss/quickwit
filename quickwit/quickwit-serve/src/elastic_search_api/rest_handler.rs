...
pub fn elastic_index_search_handler(
    search_service: Arc<dyn SearchService>,
) -> impl Filter<Extract = (impl warp::Reply,), Error = Rejection> + Clone {
    elastic_index_search_filter()
        .and(with_arg(search_service))
        .then(elastic_index_search)
        .map(|resp| BodyFormat::Json.make_rest_reply(resp))
}

async fn elastic_index_search(
    indexes: SimpleList,
    ...
    let index_id: String = indexes.0.into_iter().next().unwrap();
...
fn convert_hit(hit: quickwit_proto::Hit) -> ElasticHit {
    ...
    ElasticHit {
        ...
        score: None,
        nested: None,
        source: Source::from_string(hit.json.to_string())
