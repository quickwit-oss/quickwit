...
fn compute_term_query(
    ...
    if !field_type.is_indexed() {
        bail!("Field is not index {}", field_name);
    }
    if field_type.value_type() != Type::Json && !json_path.is_empty() {
        bail!("Field does not exist. {field_name}.{json_path}");
    }
    match *field_type {
        FieldType::U64(_) => {
...
impl Into<(String, TermQueryValue)> for TermQuery {
    fn into(self) -> (String, TermQueryValue) {
        (self.field, TermQueryValue { value: self.value })
    }
}
...
impl TryFrom<HashMap<String, TermQueryValue>> for TermQuery {
    ...
    fn try_from(map: HashMap<String, TermQueryValue>) -> Result<Self, Self::Error> {
        ...
        Ok(TermQuery::from(map.into_iter().next().unwrap())) // unwrap justified by the if statement
                                                             // above.
    }
}

impl Into<HashMap<String, TermQueryValue>> for TermQuery {
    fn into(self) -> HashMap<String, TermQueryValue> {
        let (field, term_query_value) = self.into();
