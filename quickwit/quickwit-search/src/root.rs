...
async fn search_partial_hits_round_with_scroll(
    ...
    if let Some(scroll_ttl) = scroll_ttl_opt {
        ...
        let mut leaf_search_resp = search_partial_hits_round(
            searcher_context,
            &search_request,
            index_uri,
            doc_mapper_str,
        ...
        let leaf_search_resp = search_partial_hits_round(
            searcher_context,
            &search_request,
            index_uri,
            doc_mapper_str,
...
#[instrument(skip(search_request, cluster_client))]
pub(crate) async fn search_partial_hits_round(
    searcher_context: &SearcherContext,
    search_request: &SearchRequest,
    index_uri: &Uri,
    doc_mapper_str: &str,
...
pub(crate) async fn fetch_docs_round(
...
async fn root_search_aux(
    ...
    let (first_round_result, scroll_key_and_start_offset_opt): (
        LeafSearchResponse,
        Option<ScrollKeyAndStartOffset>,
    ) = search_partial_hits_round_with_scroll(
        searcher_context,
    ...
    let hits = fetch_docs_round(
        &first_round_result.partial_hits,
        &split_metadatas[..],
    ...
    let aggregation_result_json_opt = finalize_aggregation_if_any(
        &search_request,
        first_round_result.intermediate_aggregation_result,
        searcher_context,
    )?;

    Ok(SearchResponse {
        aggregation: aggregation_result_json_opt,
        num_hits: first_round_result.num_hits,
        hits,
