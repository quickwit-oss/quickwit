...
use crate::columnar_stream::{SearchSplitColumnarRequest, run_columnar_search};
...
pub trait SearchService: 'static + Send + Sync {
    ...
    /// Phase 2 of the columnar read API: opens a single split and streams its
    /// projected fast-field columns as Arrow record batches.
    ///
    /// Reads columnar fast fields only; never the row-oriented doc store. The
    /// returned stream yields 0..N batches; a split with no matches yields
    /// none. Callers already know the projected schema from the `columns`
    /// they requested, so it isn't echoed back on the stream.
...
impl SearchService for SearchServiceImpl {
    ...
    async fn search_split_columnar(
        ...
        // (see `CloudPremServiceImpl::search_split`), not here.
        let items = run_columnar_search(searcher_context, index_storage, request, doc_mapper);
