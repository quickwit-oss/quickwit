...
use quickwit_metastore::{IndexMetadataResponseExt, Metastore};
...
pub async fn root_search_stream(
    ...
    let index_metadata_request = IndexMetadataRequest::new(&search_stream_request.index_id);
    ...
    let doc_mapper = build_doc_mapper(&index_config.doc_mapping, &index_config.search_settings)
        .map_err(|error| {
            SearchError::InternalError(format!("Failed to build doc mapper. Cause: {error}"))
        })?;
    ...
    let splits_metadata = list_relevant_splits(index_uid, &search_request, metastore).await?;

    let doc_mapper_str = serde_json::to_string(&doc_mapper).map_err(|error| {
        SearchError::InternalError(format!("Failed to serialize doc mapper: Cause {error}"))
    })?;

    let index_uri: &Uri = &index_config.index_uri;
    let leaf_search_jobs: Vec<SearchJob> = splits_metadata.iter().map(SearchJob::from).collect();
...
mod tests {
    ...
    async fn test_root_search_stream_single_split() -> anyhow::Result<()> {
        ...
        let result: Vec<Bytes> = root_search_stream(request, &metastore, cluster_client)
            .await?
            .try_collect()
            .await?;
