...
// Add cache nodes in places where it sounds like they might be useful
...
fn add_cache_nodes(search_request: &mut SearchRequest, timestamp_field: Option<&str>) {
    ...
    let filter = |query_ast: &mut QueryAst| match query_ast {
        QueryAst::FieldPresence(_) => true,
        QueryAst::Range(range) => timestamp_field
            .map(|ts_field| ts_field == range.field)
            .unwrap_or(false),
    ...
    let mut musts: Vec<_> = bool_query.must.extract_if(.., filter).collect();
    let filters: Vec<_> = bool_query.filter.extract_if(.., filter).collect();
    let some_filters_removed = !musts.is_empty() || !filters.is_empty();
...
mod tests {
    ...
    use quickwit_proto::search::SortField;
    use quickwit_query::query_ast::{
