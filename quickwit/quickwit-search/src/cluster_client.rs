...
impl ClusterClient {
    ...
    pub async fn put_kv(&self, key: &[u8], payload: &[u8], ttl: Duration) {
        ...
        let successful_replication = futures::stream::iter(clients)
            .map(|mut client| {
                ...
                async move { client.put_kv(put_kv_request).await }
            })
            .buffer_unordered(TARGET_NUM_REPLICATION)
            .filter(|put_kv_successful| ready(put_kv_successful.is_ok()))
