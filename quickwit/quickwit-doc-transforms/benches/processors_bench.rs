...
use quickwit_doc_transforms::transformers::SyslogProcessor;
...
fn bench_syslog_processor_parsing(mut runner: InputGroup<Vec<ProcessedLog>>) {
    ...
    runner.register("preprocessing_pipeline", |processed_logs| {
        ...
        for mut processed_log in processed_logs.iter().cloned() {
            ...
            match pipeline.apply(&mut processed_log) {
                Ok(_) => {
                    total_processed += 1;
                    // Count custom fields as a measure of work done
                    total_processed += processed_log.custom.len() as u64;
                }
                Err(_) => {
                    // Count errors too
...
fn bench_go_integration_parsing(mut runner: InputGroup<Vec<ProcessedLog>>) {
    runner.throughput(|processed_logs| {
        processed_logs
            .iter()
            .map(|log| log.message.len())
    ...
    runner.register("go_integration_processor", |processed_logs| {
        ...
        for mut processed_log in processed_logs.iter().cloned() {
            ...
            match integrations_processor.apply(&mut processed_log) {
                Ok(_) => {
                    total_processed += 1;
...
fn main() {
    println!("Running Syslog Processor Benchmarks");

    let syslog_datasets = vec![("10k messages", generate_syslog_processed_logs(10_000))];
