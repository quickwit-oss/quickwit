        ...
        if !self.override_if_exists {
            match self.target_type {
                ...
                AttrRemapperTargetType::Attribute => {
                    if get_nested(&log.custom, self.to_path.iter()).is_some() {
                        return Ok(());
                    }
                }
            }
        ...
        match self.source_type {
            ...
            AttrRemapperTargetType::Attribute => {
                for from_path in &self.sources {
                    ...
                    let from_val_opt = get_nested(&log.custom, from_path.iter()).cloned();
...
mod tests {
    ...
    fn test_remap_tag_to_attribute_remove_original() {
        ...
        assert_eq!(
            get_nested(&log.custom, ["dest_attr"].into_iter()).cloned(),
            Some(json!("tag_value"))
        );
