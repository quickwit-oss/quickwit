...
impl CheckTypeCounters {
    ...
    fn bump(&mut self, report: &TestCaseReport) {
        ...
        if report.custom_fully_failed || !report.custom {
            self.total_custom_fully_failed += 1;
        }

        // optional checks
        if let Some(ok) = report.service {
        ...
        if let Some(ok) = report.timestamp {
            self.total_timestamp += 1;
            if ok {
                self.succ_timestamp += 1;
            }
        }
        if let Some(ok) = report.trace_id {
...
struct TestCaseReport {
    ...
    service: Option<bool>,
    /// If the status check failed or not
    status: Option<bool>,
    /// If the timestamp check failed or not
    timestamp: Option<bool>,
    /// If the trace_id check failed or not
    trace_id: Option<bool>,
}
impl TestCaseReport {
    fn iter_checks(&self) -> impl Iterator<Item = (&str, bool)> {
        let mut checks = vec![("custom", self.custom), ("message", self.message)];
...
fn run_test_case(
    ...
    let mut expected_custom = test_case.result.custom.clone();
    normalize_numbers_in_obj(&mut processed_log.custom);
    normalize_numbers_in_obj(&mut expected_custom);

    report.custom = processed_log.custom != expected_custom;
    report.message = processed_log.message == test_case.result.message;
    ...
    if let Some(timestamp) = &test_case.result.timestamp {
        report.timestamp = Some(processed_log.timestamp == *timestamp);
    }
    if let Some(_trace_id) = &test_case.result.trace_id {
        report.trace_id = Some(processed_log.trace_id == test_case.result.trace_id);
    }
    report
}
