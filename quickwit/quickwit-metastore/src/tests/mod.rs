...
pub async fn test_metastore_add_source<MetastoreToTest: MetastoreService + DefaultForTest>() {
        ...
        metastore
            .add_source(
                AddSourceRequest::try_from_source_config(
                    IndexUid::from_index_id_with_random_uid("index-not-found"),
                    source.clone()
                )
                .unwrap()
            )
    ...
    assert!(matches!(
        metastore
            .add_source(
                AddSourceRequest::try_from_source_config(
                    IndexUid::from_index_id_with_random_uid(index_id),
                    source
                )
                .unwrap()
            )
...
pub async fn test_metastore_delete_source<MetastoreToTest: MetastoreService + DefaultForTest>() {
    ...
    assert!(matches!(
        metastore
            .delete_source(DeleteSourceRequest {
                index_uid: IndexUid::from_index_id_with_random_uid("index-not-found").to_string(),
                source_id: source_id.to_string()
            })
            .await
            .unwrap_err(),
    ...
    assert!(matches!(
        metastore
            .delete_source(DeleteSourceRequest {
                index_uid: IndexUid::from_index_id_with_random_uid(index_id).to_string(),
                source_id: source_id.to_string()
            })
            .await
            .unwrap_err(),
...
pub async fn test_metastore_publish_splits<MetastoreToTest: MetastoreService + DefaultForTest>() {
    ...
    let index_uid = IndexUid::from_index_id_with_random_uid(index_id.clone());
    let index_uri = format!("ram:///indexes/{index_id}");
    let index_config = IndexConfig::for_test(&index_id, &index_uri);
...
>() {
    ...
    let error = metastore
        .create_delete_task(DeleteQuery {
            index_uid: IndexUid::from_index_id_with_random_uid("does-not-exist:12345").to_string(),
            ..delete_query.clone()
        })
        .await
        .unwrap_err();
