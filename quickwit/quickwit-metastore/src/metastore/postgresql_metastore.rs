...
use crate::{
    IndexMetadata, IndexUid, ListSplitsQuery, Metastore, MetastoreError, MetastoreFactory,
    ...
    E: sqlx::Executor<'a, Database = Postgres>,
{
    ...
    .bind(index_uid.incarnation_id.to_string())
    .fetch_optional(executor)
    .await
    .map_err(|error| MetastoreError::DbError {
        message: error.to_string(),
...
fn build_query_filter(mut sql: String, query: &ListSplitsQuery) -> String {
    sql.push_str(" WHERE index_id = $1 AND incarnation_id = $2");
    ...
    MetastoreError: From<E>,
{
    ...
    let update_index_res = sqlx::query(
        ...
        WHERE index_id = $2 AND incarnation_id = $3
        "#,
    )
...
impl Metastore for PostgresqlMetastore {
    ...
    async fn create_index(&self, index_config: IndexConfig) -> MetastoreResult<IndexUid> {
        ...
        sqlx::query(
            "INSERT INTO indexes (index_id, index_metadata_json, incarnation_id) VALUES ($1, $2, \
             $3)",
        )
        .bind(index_metadata.index_id())
        .bind(&index_metadata_json)
        .bind(index_metadata.incarnation_id.to_string())
    ...
    async fn delete_index(&self, index_uid: IndexUid) -> MetastoreResult<()> {
        let delete_res =
            sqlx::query("DELETE FROM indexes WHERE index_id = $1 AND incarnation_id = $2")
                .bind(&index_uid.index_id)
                .bind(index_uid.incarnation_id.to_string())
                .execute(&self.connection_pool)
                .await?;
    ...
    async fn stage_splits(
        ...
        run_with_tx!(self.connection_pool, tx, {
            let upserted_split_ids: Vec<String> = sqlx::query_scalar(r#"
                INSERT INTO splits
                    (split_id, time_range_start, time_range_end, tags, split_metadata_json, delete_opstamp, split_state, index_id, incarnation_id)
                SELECT
                    split_id,
                    time_range_start,
                    time_range_end,
    ...
    async fn list_splits(&self, query: ListSplitsQuery) -> MetastoreResult<Vec<Split>> {
        ...
        let pg_splits = sqlx::query_as::<_, PgSplit>(&sql)
            .bind(&query.index_uid.index_id)
            .bind(query.index_uid.incarnation_id.to_string())
            .fetch_all(&self.connection_pool)
            .await?;
        ...
        {
            return Err(MetastoreError::IndexDoesNotExist {
                index_id: query.index_uid.index_id,
            });
        }
        pg_splits
            .into_iter()
    ...
    async fn create_delete_task(&self, delete_query: DeleteQuery) -> MetastoreResult<DeleteTask> {
        ...
        let (create_timestamp, opstamp): (sqlx::types::time::PrimitiveDateTime, i64) =
            sqlx::query_as(
                r#"
            INSERT INTO delete_tasks (index_id, delete_query_json, incarnation_id) VALUES ($1, $2, $3)
            RETURNING create_timestamp, opstamp
            "#,
            )
            .bind(&delete_query.index_id)
            .bind(&delete_query_json)
            .bind(&delete_query.incarnation_id)
            .fetch_one(&self.connection_pool)
            .await
...
mod tests {
    ...
    use crate::{IndexUid, ListSplitsQuery, SplitState};
    ...
    fn test_single_sql_query_builder() {
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND split_state IN ('Staged')"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND split_state IN ('Published')"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND update_timestamp < to_timestamp(51)"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND create_timestamp <= to_timestamp(55)"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND delete_opstamp >= 4"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND (time_range_end > 45 OR \
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND (time_range_start < 45 OR \
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND (NOT ($$tag-2$$ = ANY(tags)))"
        );
    }
    ...
    fn test_combination_sql_query_builder() {
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND (time_range_end > 0 OR \
             time_range_end IS NULL) AND (time_range_start < 40 OR time_range_start IS NULL)"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND (time_range_end > 45 OR \
             time_range_end IS NULL) AND delete_opstamp > 0"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND update_timestamp < to_timestamp(51) \
             AND create_timestamp <= to_timestamp(63)"
        );
        ...
        assert_eq!(
            sql,
            " WHERE index_id = $1 AND incarnation_id = $2 AND ($$tag-1$$ = ANY(tags)) AND \
