...
/// Sea-query table identifier for sketch_splits.
#[derive(Iden, Clone, Copy)]
pub enum SketchSplits {
    Table,
    SplitId,
    SplitState,
    IndexUid,
...
mod tests {
    use super::*;
    use quickwit_parquet_engine::split::{ParquetSplitId, ParquetSplitKind, TimeRange};
    ...
    fn test_insertable_from_metadata_with_compaction_fields() {
        ...
        let insertable =
            InsertableParquetSplit::from_metadata(&metadata, SplitState::Published)
                .expect("conversion should succeed");
