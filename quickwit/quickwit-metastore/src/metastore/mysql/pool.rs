...
use sqlx::{
    Acquire, Database, Describe, Either, Error, Execute, Executor, MySql, Pool, Transaction,
};
...
impl TrackedPool<MySql> {
    pub fn new(inner_pool: Pool<MySql>) -> Self {
        Self { inner_pool }
    }

    pub fn set_connect_options(&self, options: sqlx::mysql::MySqlConnectOptions) {
        self.inner_pool.set_connect_options(options);
    }
}
...
impl<'a, DB: Database> Acquire<'a> for &TrackedPool<DB> {
    ...
    fn acquire(self) -> BoxFuture<'static, Result<Self::Connection, Error>> {
        ...
        Box::pin(async move {
            let mut gauge_guard = GaugeGuard::from_gauge(&MYSQL_METRICS.acquire_connections);
            gauge_guard.add(1);
