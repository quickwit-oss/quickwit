...
| `.trace_id`                          | canonical i64 (lower 64)                                                          | Unsigned decimal of the lower 64. **UI-compat workaround:** `meta._dd.p.tid` is stripped at the start of `span_to_schema` so the 32-char hex branch never fires today; remove that strip once the UI supports 128-bit hex trace IDs. |
