...
pub trait Hello: std::fmt::Debug + Send + Sync + 'static {
    ...
    ///Says goodbye.
    async fn goodbye(&self, request: GoodbyeRequest) -> crate::HelloResult<GoodbyeResponse>;
    ///Ping pong.
    async fn ping(
        &self,
...
impl tower::Service<quickwit_common::ServiceStream<PingRequest>> for InnerHelloClient {
    ...
    fn call(&mut self, request: quickwit_common::ServiceStream<PingRequest>) -> Self::Future {
        let svc = self.clone();
...
impl HelloTowerLayerStack {
    ...
    where
        ...
        <L as tower::Layer<
            quickwit_common::tower::BoxService<HelloRequest, HelloResponse, crate::HelloError>,
        >>::Service: tower::Service<HelloRequest, Response = HelloResponse, Error = crate::HelloError>
            + Clone
            + Send
            + Sync
            + 'static,
        ...
        <L as tower::Layer<
            quickwit_common::tower::BoxService<GoodbyeRequest, GoodbyeResponse, crate::HelloError>,
        >>::Service: tower::Service<GoodbyeRequest, Response = GoodbyeResponse, Error = crate::HelloError>
            + Clone
            + Send
            + Sync
            + 'static,
        ...
        <<L as tower::Layer<
            quickwit_common::tower::BoxService<
                ...
                crate::HelloError,
            >,
        >>::Service as tower::Service<quickwit_common::ServiceStream<PingRequest>>>::Future:
            Send + 'static,
    {
        self.hello_layers
    ...
    where
        L: tower::Layer<
            ...
            + Sync
            + 'static,
        L::Service: tower::Service<HelloRequest, Response = HelloResponse, Error = crate::HelloError>
            + Clone
            + Send
            + Sync
            + 'static,
    ...
    where
        L: tower::Layer<
            ...
            + Sync
            + 'static,
        L::Service: tower::Service<GoodbyeRequest, Response = GoodbyeResponse, Error = crate::HelloError>
            + Clone
            + Send
            + Sync
            + 'static,
    ...
    where
        ...
        L::Service: tower::Service<
            ...
            + Sync
            + 'static,
        <L::Service as tower::Service<quickwit_common::ServiceStream<PingRequest>>>::Future:
            Send + 'static,
    {
        self.ping_layers
            .push(quickwit_common::tower::BoxLayer::new(layer));
    ...
    pub fn build_from_channel(
        ...
        let client =
            HelloClient::from_channel(addr, channel, max_message_size, compression_encoding_opt);
    ...
    where
        ...
        HelloMailbox<A>: Hello,
    {
        let inner_client = InnerHelloClient(std::sync::Arc::new(HelloMailbox::new(mailbox)));
        self.build_from_inner_client(inner_client)
    }
    #[cfg(any(test, feature = "testsuite"))]
    ...
    fn build_from_inner_client(self, inner_client: InnerHelloClient) -> HelloClient {
        let hello_svc = self.hello_layers.into_iter().rev().fold(
            quickwit_common::tower::BoxService::new(inner_client.clone()),
            |svc, layer| layer.layer(svc),
        );
        let goodbye_svc = self.goodbye_layers.into_iter().rev().fold(
            quickwit_common::tower::BoxService::new(inner_client.clone()),
            |svc, layer| layer.layer(svc),
        );
        let ping_svc = self.ping_layers.into_iter().rev().fold(
            quickwit_common::tower::BoxService::new(inner_client.clone()),
            |svc, layer| layer.layer(svc),
        );
    ...
    T::Future: Send,
{
    async fn hello(&self, request: HelloRequest) -> crate::HelloResult<HelloResponse> {
        self.inner
            ...
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, HelloRequest::rpc_name())
            })
    }
    async fn goodbye(&self, request: GoodbyeRequest) -> crate::HelloResult<GoodbyeResponse> {
        self.inner
            ...
            .map(|response| response.into_inner())
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, GoodbyeRequest::rpc_name())
            })
    }
    async fn ping(
        &self,
        ...
        self.inner
            ...
            .map(|response| {
                ...
                stream.map_err(|status| {
                    crate::error::grpc_status_to_service_error(status, PingRequest::rpc_name())
                })
            })
            .map_err(|status| {
                crate::error::grpc_status_to_service_error(status, PingRequest::rpc_name())
...
pub mod hello_grpc_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub async fn hello(
            ...
            req.extensions_mut()
                .insert(GrpcMethod::new("hello.Hello", "Hello"));
        ...
        pub async fn goodbye(
            ...
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("hello.Hello", "Goodbye"));
            self.inner.unary(req, path, codec).await
        }
        /// Ping pong.
        pub async fn ping(
        ...
        > {
            ...
            req.extensions_mut()
                .insert(GrpcMethod::new("hello.Hello", "Ping"));
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/hello.Hello/Hello" => {
                    ...
                    impl<T: HelloGrpc> tonic::server::UnaryService<super::HelloRequest> for HelloSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move { <T as HelloGrpc>::hello(&inner, request).await };
                            Box::pin(fut)
                        }
                    }
                ...
                "/hello.Hello/Goodbye" => {
                    ...
                    impl<T: HelloGrpc> tonic::server::UnaryService<super::GoodbyeRequest> for GoodbyeSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut =
                                async move { <T as HelloGrpc>::goodbye(&inner, request).await };
                            Box::pin(fut)
                        }
                    }
                    let accept_compression_encodings = self.accept_compression_encodings;
                ...
                "/hello.Hello/Ping" => {
                    ...
                    impl<T: HelloGrpc> tonic::server::StreamingService<super::PingRequest> for PingSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move { <T as HelloGrpc>::ping(&inner, request).await };
                            Box::pin(fut)
                        }
                    }
                    let accept_compression_encodings = self.accept_compression_encodings;
