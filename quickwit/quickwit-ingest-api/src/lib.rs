...
pub static INGEST_API_SERVICE_MAILBOXES: Lazy<Mutex<HashMap<PathBuf, Mailbox<IngestApiService>>>> =
...
pub async fn init_ingest_api(
    ...
    let actor = IngestApiService::with_queues_dir(
        queues_dir_path,
    ...
    let (mailbox, _handle) = universe.spawn_builder().spawn(actor);
    guard.insert(queues_dir_path.to_path_buf(), mailbox.clone());
    Ok(mailbox)
}
...
mod tests {
    ...
    use super::*;
    use crate::ingest_api_service::GetPartitionId;
        ...
        let tempdir = tempfile::tempdir().unwrap();
        {
            ...
            let ingest_api_service =
                init_ingest_api(&universe, &queues_dir_path, &IngestApiConfig::default())
                    .await
                    .unwrap();
            let partition_id = ingest_api_service
                .ask_for_res(GetPartitionId {})
                .await
                .unwrap();
    ...
    async fn test_get_ingest_multiple_index_api_service() {
        ...
        let mut ingest_api_service =
            init_ingest_api(&universe, tempdir.path(), &IngestApiConfig::default())
                .await
                .unwrap();
        ingest_api_service
            .ask_for_res(CreateQueueRequest {
