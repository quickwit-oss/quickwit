...
use chitchat::{ChitchatId, Version};
...
impl ClusterService for Cluster {
    async fn fetch_cluster_state(
        ...
        let digest: HashMap<ChitchatId, Version> = request
            .digest
            .expect("`digest` should be a required field")
            .node_digests
            .into_iter()
            .map(|node_digest| {
                let proto_chitchat_id = node_digest
                    .chitchat_id
                    .expect("`chitchat_id` should be a required field");
                let chitchat_id = ChitchatId {
                    node_id: proto_chitchat_id.node_id,
                ...
                (chitchat_id, node_digest.max_version)
            })
            .collect();
        ...
        for (chitchat_id, node_state) in chitchat_guard.node_states() {
            ...
            let max_version_opt = digest.get(chitchat_id).copied();
            let key_values: Vec<VersionedKeyValue> = node_state
                .key_values()
                .filter(|(_, value)| {
                    max_version_opt
                        .map(|max_version| value.version > max_version)
                        .unwrap_or(true)
                })
                .map(|(key, value)| VersionedKeyValue {
                    key: key.to_string(),
...
mod tests {
    ...
    use quickwit_proto::cluster::{Digest, NodeDigest};
    ...
    async fn test_fetch_cluster_state() {
        ...
        cluster.set_self_key_value("baz", "qux").await;

        let digest = Digest {
            node_digests: vec![NodeDigest {
                chitchat_id: Some(chitchat_id),
                max_version: node_state.max_version,
            }],
        };
        let fetch_cluster_state_request = FetchClusterStateRequest {
        ...
        let fetch_cluster_state_response = cluster
            .fetch_cluster_state(fetch_cluster_state_request)
            .await
            .unwrap();
        assert_eq!(
        ...
        let node_state = &fetch_cluster_state_response.node_states[0];
        assert_eq!(node_state.key_values.len(), 1);
        assert_eq!(node_state.key_values[0].key, "baz");
        assert_eq!(node_state.key_values[0].value, "qux");
    }
}
