...
/// Trait used by a [`ClientPool`] to build a new client
/// when new nodes are joining the cluster.
/// The `service` is used to select the relevant node
/// from which a client is built.
#[async_trait]
pub trait ServiceClientBuilder: Clone + Send + Sync + 'static {
    fn service() -> QuickwitService;
    async fn build_client(addr: SocketAddr) -> anyhow::Result<Self>;
}

/// Service client pool.
/// Used for searchers and indexers.
/// Clients are stored in a map gRPC address -> client. The pool can be dynamically updated
/// when created from a channel monitoring cluster members changes.
#[derive(Clone)]
pub struct ServiceClientPool<T: ServiceClientBuilder> {
    clients: Arc<RwLock<HashMap<SocketAddr, T>>>,
}

impl<T: ServiceClientBuilder> Default for ServiceClientPool<T> {
    fn default() -> Self {
        Self {
...
impl<T: ServiceClientBuilder> ServiceClientPool<T> {
...
async fn update_client_map<T: ServiceClientBuilder>(
...
mod tests {
    ...
    use crate::{ClusterMember, ServiceClientBuilder, ServiceClientPool};

    #[async_trait]
    impl ServiceClientBuilder for SearchServiceClient<Channel> {
        async fn build_client(addr: SocketAddr) -> anyhow::Result<Self> {
            let uri = Uri::builder()
                .scheme("http")
