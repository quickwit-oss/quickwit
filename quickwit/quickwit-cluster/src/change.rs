...
async fn compute_cluster_change_events_on_updated(
    ...
    let updated_node_id = updated_node.node_id.clone();
...
pub(crate) mod tests {
    ...
    async fn test_compute_cluster_change_events_on_added() {
        ...
        }
        {
            ...
            let node = previous_nodes
                .get(new_chitchat_id.node_id.as_ref())
                .unwrap();
        ...
        }
        {
            ...
            assert_eq!(
                previous_nodes
                    .get(rejoined_chitchat_id.node_id.as_ref())
                    .unwrap(),
                rejoined_node
            );
    ...
    async fn test_compute_cluster_change_events_on_updated() {
        ...
        let self_chitchat_id = ChitchatId::for_local_test(self_port);
        {
            ...
            let updated_node_id: NodeId = NodeId::from_arc_str(updated_chitchat_id.node_id.clone());
            ...
            assert_eq!(
                previous_nodes
                    .get(updated_chitchat_id.node_id.as_ref())
                    .unwrap(),
                &node
            );
        }
    ...
    async fn test_compute_cluster_change_events_on_removed() {
        ...
        }
        {
            ...
            let event_opt =
                compute_cluster_change_events_on_removed(&removed_chitchat_id, &mut previous_nodes);
            assert!(event_opt.is_none());
            assert!(!previous_nodes.contains_key(removed_chitchat_id.node_id.as_ref()));
        }
        {
        ...
        }
        {
            ...
            rejoined_chitchat_id.generation_id += 1;
            let rejoined_node_id: NodeId =
                NodeId::from_arc_str(rejoined_chitchat_id.node_id.clone());
    ...
    async fn test_compute_cluster_change_events() {
        ...
        let self_port = 1234;
        let self_chitchat_id = ChitchatId::for_local_test(self_port);
        let self_node_id: NodeId = NodeId::from_arc_str(self_chitchat_id.node_id.clone());
        {
            let mut previous_nodes = BTreeMap::default();
