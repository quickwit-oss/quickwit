...
mod tests {
    ...
    fn test_deserialize_pipeline_shards_sorts() {
        assert_eq!(deserialize_pipeline_shards("[2,1]").unwrap(), vec![vec![1,2]]);
        assert_eq!(deserialize_pipeline_shards("[1][]").unwrap(), vec![vec![], vec![1]]);
        assert_eq!(deserialize_pipeline_shards("[3][2]").unwrap(), vec![vec![2], vec![3]]);
    }

    fn test_serialize_indexing_tasks_aux(indexing_tasks: &[IndexingTask], node_state: &mut NodeState) {
        set_indexing_tasks_in_node_state(indexing_tasks, node_state);
    ...
    fn test_serialize_indexing_tasks() {
        ...
        test_serialize_indexing_tasks_aux(&[IndexingTask {
