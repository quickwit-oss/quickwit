...
impl Throttle {
    ...
    pub fn consume(&self, num_units: u64) {
        ...
        if should_wait {
            let _ = self.rw_lock.blocking_read();
        }
        self.num_units_consumed
            .fetch_add(num_units, Ordering::Relaxed);
    }

    /// Async version of consume (It is not used at the moment.)
    #[allow(dead_code)]
    pub async fn consume_async(&self, num_units: u64) {
        let should_wait = self.should_wait.load(Ordering::Relaxed);
        if should_wait {
            let _ = self.rw_lock.read().await;
        }
        self.num_units_consumed
            .fetch_add(num_units, Ordering::Relaxed);
    }
...
fn apply_cooldown(throttle: &Throttle, cool_down_duration: Duration) {
    let _wlock = throttle.rw_lock.blocking_write();
    throttle.should_wait.store(true, Ordering::Relaxed);
    std::thread::sleep(cool_down_duration);
    throttle.should_wait.store(false, Ordering::Relaxed);
}

fn throttling_control_loop(
    ...
    while let Some(throttle) = throttle_weak.upgrade() {
        ...
        if credit < 0 {
            ...
            apply_cooldown(&throttle, cool_down_duration);
        } else {
            // Let everyone unhinged for 1 full second.
            std::thread::sleep(throttling_reactivity);
        }
    }
}
...
pub fn create_throttle(
    ...
    std::thread::spawn(move || {
        throttling_control_loop(throttle_weak, units_per_sec, throttling_reactivity, counter)
    });
    throttle
}
...
mod tests {
    ...
    fn test_counter() -> IntCounter {
        IntCounter::new("test_counter", "Generates a fake counter just for tests.").unwrap()
    }

    #[tokio::test]
    async fn test_sync_throttling() {
        let throttler = create_throttle(1_000, Duration::from_millis(10), test_counter());
    ...
    async fn test_async_throttling() {
        let throttler = create_throttle(1_000, Duration::from_millis(20), test_counter());
        let now = Instant::now();
        // We try to consume 400 at a pace of 4_000 / secs.
        // Because we are throttled, it will actually take around 400ms.
        for _ in 0..100 {
    ...
    async fn test_credit_do_accumulate() {
        let throttler = create_throttle(100_000, Duration::from_millis(100), test_counter());
        let now = Instant::now();
        // We try to consume intermittently.
        //
        // - 100 at a pace of 4000/secs
