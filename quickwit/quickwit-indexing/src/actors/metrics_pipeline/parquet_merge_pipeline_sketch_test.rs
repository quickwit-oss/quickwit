...
async fn test_merge_pipeline_end_to_end_sketches() {
    ...
    let metrics_meta_a =
        super::parquet_merge_pipeline_test::make_test_split_metadata("sketch-a", 50, 0, 100, "cpu.usage");
    let size_a = write_test_parquet_file(temp_dir.path(), "sketch-a.parquet", &batch_a, &metrics_meta_a);
    ...
    let metrics_meta_b =
        super::parquet_merge_pipeline_test::make_test_split_metadata("sketch-b", 50, 0, 200, "mem.usage");
    let size_b = write_test_parquet_file(temp_dir.path(), "sketch-b.parquet", &batch_b, &metrics_meta_b);
    ...
    mock_metastore
        ...
        .returning(move |request: StageSketchSplitsRequest| {
            let splits = request
    ...
    assert_eq!(staged.len(), 1, "exactly one merged sketch split should be staged");
    ...
    assert_eq!(merged_meta.num_rows, 100, "merged sketch split must have 100 rows");

    let expected_metrics: HashSet<String> =
