    ...
    async fn test_merge_planner_with_stable_custom_merge_policy() -> anyhow::Result<()> {
            ...
            universe.spawn_builder().spawn(merge_planner);
        {
            ...
            let message = NewSplits {
                new_splits: vec![
                    split_metadata_for_test(&index_uid, "1_1", 1, Ulid(0), 2500, 0),
                    split_metadata_for_test(&index_uid, "1v2_1", 1, Ulid(2), 2500, 0),
                    split_metadata_for_test(&index_uid, "1_2", 2, Ulid(0), 3000, 0),
                ],
            };
        ...
        }
        {
            ...
            let message = NewSplits {
                new_splits: vec![
                    split_metadata_for_test(&index_uid, "2_1", 1, Ulid(0), 2000, 0),
                    split_metadata_for_test(&index_uid, "2v2_1", 1, Ulid(2), 2500, 0),
                    split_metadata_for_test(&index_uid, "1_2", 2, Ulid(0), 3000, 0),
                ],
            };
            merge_planner_mailbox.send_message(message).await?;
        ...
        }
        {
            ...
            let message = NewSplits {
                new_splits: vec![
                    split_metadata_for_test(&index_uid, "3_1", 1, Ulid(0), 1500, 0),
                    split_metadata_for_test(&index_uid, "4_1", 1, Ulid(0), 1000, 0),
                    split_metadata_for_test(&index_uid, "3v2_1", 1, Ulid(2), 1500, 0),
                    split_metadata_for_test(&index_uid, "2_2", 2, Ulid(0), 2000, 0),
                    split_metadata_for_test(&index_uid, "3_2", 2, Ulid(0), 4000, 0),
                ],
            };
            merge_planner_mailbox.send_message(message).await?;
            ...
            assert!(first_merge_operation
                .splits
                .iter()
                .all(|split| split.partition_id == 1 && split.doc_mapper_version == Ulid(0)));
            ...
            assert!(second_merge_operation
                .splits
                .iter()
                .all(|split| split.partition_id == 1 && split.doc_mapper_version == Ulid(2)));
            ...
            assert!(third_merge_operation
                .splits
                .iter()
                .all(|split| split.partition_id == 2 && split.doc_mapper_version == Ulid(0)));
        }
