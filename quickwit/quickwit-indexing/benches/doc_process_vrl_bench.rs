...
fn doc_processor_light_transform() -> DocProcessor {
    ...
    "#;
    let vrl_settings = VrlSettings::new_for_test(source);
...
fn bench_light_json(c: &mut Criterion) {
    ...
    group.throughput(criterion::Throughput::Bytes(JSON_LIGHT_TRANSFORM.len() as u64));
...
fn bench_heavy_json(c: &mut Criterion) {
    ...
    group.throughput(criterion::Throughput::Bytes(JSON_HEAVY_TRANSFORM.len() as u64));
