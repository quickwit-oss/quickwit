...
use crate::query_ast::exists_query::ExistsQuery;
...
pub trait QueryAstVisitor<'a> {
    ...
    fn visit(&mut self, query_ast: &'a QueryAst) -> Result<(), Self::Err> {
        match query_ast {
            ...
            QueryAst::Exists(exists) => self.visit_exists(exists),
        }
    }
    ...
    fn visit_exists(&mut self, _exists_query: &'a ExistsQuery) -> Result<(), Self::Err> {
        Ok(())
    }
}
