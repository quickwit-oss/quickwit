...
static REALLOC_SIZE: AtomicI64 = AtomicI64::new(0);
...
pub fn start_profiling(min_alloc_size_for_backtrace: usize) -> Result<(), ()> {
    ...
    REALLOC_SIZE.store(0, Ordering::SeqCst);
    Ok(())
}
...
pub fn stop_profiling() -> Result<JemallocProfile, ()> {
    ...
    Ok(JemallocProfile {
        ...
        realloc_size: REALLOC_SIZE.load(Ordering::SeqCst),
    })
}
...
fn track_realloc_call(current_layout: Layout, new_size: usize) {
    ...
    REALLOC_SIZE.fetch_add(alloc_delta, Ordering::Relaxed);
}
...
mod prefix_helper {
    ...
    impl PrefixWriteMatcher {
        ...
        fn matched(&self) -> bool {
            self.matched == self.prefix.len()
        }
    }

    impl Write for PrefixWriteMatcher {
        fn write_str(&mut self, new_slice: &str) -> std::fmt::Result {
