...
impl Handler<DebouncedPruneShardsRequest> for ControlPlane {
    ...
    async fn handle(
        ...
        let Some(metastore_request) = request.request else {
            return Ok(Ok(EmptyResponse {}));
        };
        let debouncer: &mut Debouncer = self
        ...
        debouncer.self_send_with_cooldown(metastore_request, ctx);
        Ok(Ok(EmptyResponse {}))
    }
}

/// This message is generated by the debouncer, after the control plane API is
/// called using [`DebouncedPruneShardsRequest`]
#[async_trait]
