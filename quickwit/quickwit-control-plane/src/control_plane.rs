...
impl ControlPlane {
    ...
    fn debug_info(&self) -> JsonValue {
        // Builds the union of ingesters tracked by the model and pool.
        ...
        for (ingester_id, ingester) in self.model.ingesters() {
            if let Entry::Vacant(entry) = ingester.entry(ingester_id.clone()) {
                let ingester_json = json!({
                    "available": false,
