...
mod tests {
    ...
    fn test_build_physical_indexing_plan_simple() {
        let source_1 = SourceUid {
            index_uid: IndexUid::new("index-1"),
            source_id: "source1".to_string(),
        };
        let source_2 = SourceUid {
            index_uid: IndexUid::new("index-2"),
                ...
                for task in tasks {
                    ...
                    match &source_to_schedule.source_type {
                        ...
                        SourceToScheduleType::Sharded {
                            ...
                            load_in_node += *load_per_shard * task.shard_ids.len() as u32;
                        }
                        SourceToScheduleType::NonSharded {
                            num_pipelines: _ ,
                            load_per_pipeline,
                        } => {
                            load_in_node += *load_per_pipeline;
                        }
                    }
                }
