...
fn compute_load_per_shard(shard_entries: &[&ShardEntry]) -> NonZeroU32 {
    ...
    let average_throughput_per_shard: u32 = shard_entries.iter()
        .map(|shard_entry| u32::from(shard_entry.ingestion_rate.0))
        .sum::<u32>().div_ceil(num_shards);
    let num_cpu_millis =
        (PIPELINE_FULL_CAPACITY.cpu_millis() * average_throughput_per_shard) / 20;
    ...
    for (source_uid, source_config) in model.source_configs() {
        ...
        match source_config.source_type() {
            ...
            SourceType::IngestV2 => {
                ...
                let shard_ids = shard_entries.iter()
