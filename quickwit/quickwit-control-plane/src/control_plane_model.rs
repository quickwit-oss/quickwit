...
pub struct ControlPlaneModel {
    pub index_uid_table: FnvHashMap<IndexId, IndexUid>,
...
impl ControlPlaneModel {
    ...
    pub async fn load_from_metastore(
        ...
        self.index_table.reserve(index_metadatas.len());
    ...
    pub(crate) fn fetch_source_configs(&self) -> FnvHashMap<SourceUid, SourceConfig> {
        self.index_table
            .values()
            .into_iter()
...
struct ShardTable {
    table_entries: FnvHashMap<crate::SourceUid, ShardTableEntry>,
}
...
impl ShardTable {
    ...
    fn add_source(&mut self, index_uid: &IndexUid, source_id: &SourceId) {
        let source_uid = crate::SourceUid {
