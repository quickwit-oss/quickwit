...
impl TimeZone {
    ...
    pub fn utc_offset(&self) -> Result<UtcOffset, String> {
        match self {
            TimeZone::Local => local_offset(),
            TimeZone::Offset(offset) => parse_offset(offset),
        }
    }
}

fn local_offset() -> Result<UtcOffset, String> {
    static LOCAL_OFFSET: Lazy<Result<UtcOffset, String>> =
        Lazy::new(|| UtcOffset::current_local_offset().map_err(|error| error.to_string()));
...
fn parse_offset(offset: &str) -> Result<UtcOffset, String> {
    ...
    let hours = captures
        .name("hours")
        .expect("The `hours` capture should match.")
        .as_str()
        .parse::<i32>()
        .expect("The `hours` capture should be an integer.");
    let minutes = captures
        .name("minutes")
        .map(|minutes| minutes.as_str())
        .unwrap_or("0")
        .parse::<i32>()
        .expect("The `minutes` capture should be an integer.");
...
impl<'de> Deserialize<'de> for TimeZone {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let time_zone_str = String::deserialize(deserializer)?;
