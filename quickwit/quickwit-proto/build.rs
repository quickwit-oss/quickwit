...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // `SearchResponse` is ~700 bytes; box it so the `AnyResponse::Response` oneof
    // (~20 small variants) isn't bloated by a single fat variant.
    cloudprem_prost_config.boxed(".cloudprem.AnyResponse.response.root_search");
