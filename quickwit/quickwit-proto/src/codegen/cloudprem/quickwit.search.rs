...
impl RpcName for GetLoadRequest {
    fn rpc_name() -> &'static str {
        "get_load"
    }
}
...
pub trait SearchService: std::fmt::Debug + Send + Sync + 'static {
    ...
    ) -> crate::cloudprem::CloudPremResult<GetLoadResponse>;
}
#[derive(Debug, Clone)]
...
impl SearchServiceClient {
    ...
    pub fn from_channel(
        ...
        let mut client = search_service_grpc_client::SearchServiceGrpcClient::new(
                channel,
            )
            .max_decoding_message_size(max_message_size.0 as usize)
            .max_encoding_message_size(max_message_size.0 as usize);
...
impl SearchService for SearchServiceClient {
    ...
    #[tracing::instrument(skip_all, name = "search.get_load")]
    async fn get_load(
        &self,
        request: GetLoadRequest,
    ) -> crate::cloudprem::CloudPremResult<GetLoadResponse> {
        self.inner.0.get_load(request).await
    }
}
#[cfg(any(test, feature = "testsuite"))]
pub mod mock_search_service {
    ...
    impl SearchService for MockSearchServiceWrapper {
        ...
        async fn get_load(
            &self,
            request: super::GetLoadRequest,
        ) -> crate::cloudprem::CloudPremResult<super::GetLoadResponse> {
            self.inner.lock().await.get_load(request).await
        }
    }
}
...
impl tower::Service<GetLoadRequest> for InnerSearchServiceClient {
    type Response = GetLoadResponse;
    type Error = crate::cloudprem::CloudPremError;
    type Future = BoxFuture<Self::Response, Self::Error>;
    fn poll_ready(
        &mut self,
    ...
    fn call(&mut self, request: GetLoadRequest) -> Self::Future {
        let svc = self.clone();
        let fut = async move { svc.0.get_load(request).await };
        Box::pin(fut)
    }
}
...
struct SearchServiceTowerServiceStack {
    ...
    search_plan_svc: quickwit_common::tower::BoxService<
        ...
        crate::cloudprem::CloudPremError,
    >,
    get_load_svc: quickwit_common::tower::BoxService<
        GetLoadRequest,
        GetLoadResponse,
        crate::cloudprem::CloudPremError,
    >,
...
impl SearchService for SearchServiceTowerServiceStack {
    ...
    async fn get_load(
        ...
        self.get_load_svc.clone().ready().await?.call(request).await
    }
}
...
type SearchPlanLayer = quickwit_common::tower::BoxLayer<
    ...
    crate::cloudprem::CloudPremError,
>;
type GetLoadLayer = quickwit_common::tower::BoxLayer<
    quickwit_common::tower::BoxService<
        GetLoadRequest,
        GetLoadResponse,
        crate::cloudprem::CloudPremError,
...
pub struct SearchServiceTowerLayerStack {
    ...
    get_load_layers: Vec<GetLoadLayer>,
}
impl SearchServiceTowerLayerStack {
    ...
    where
        ...
        <<L as tower::Layer<
            quickwit_common::tower::BoxService<
                ...
                crate::cloudprem::CloudPremError,
            >,
        >>::Service as tower::Service<GetLoadRequest>>::Future: Send + 'static,
    {
        ...
        self.search_plan_layers
            .push(quickwit_common::tower::BoxLayer::new(layer.clone()));
        self.get_load_layers.push(quickwit_common::tower::BoxLayer::new(layer.clone()));
        self
    }
    ...
    pub fn stack_get_load_layer<L>(mut self, layer: L) -> Self
    where
        L: tower::Layer<
                quickwit_common::tower::BoxService<
                    GetLoadRequest,
        ...
        L::Service: tower::Service<
                ...
                Error = crate::cloudprem::CloudPremError,
            > + Clone + Send + Sync + 'static,
        <L::Service as tower::Service<GetLoadRequest>>::Future: Send + 'static,
    {
        self.get_load_layers.push(quickwit_common::tower::BoxLayer::new(layer));
        self
    }
    ...
    pub fn build_from_channel(
        ...
        let client = SearchServiceClient::from_channel(
            addr,
            channel,
            max_message_size,
...
where
    ...
    >: tower::Service<
        ...
        + tower::Service<
            ...
            Response = GetLoadResponse,
            Error = crate::cloudprem::CloudPremError,
            Future = BoxFuture<GetLoadResponse, crate::cloudprem::CloudPremError>,
        >,
{
    async fn root_search(
        &self,
    ...
    T::Future: Send,
{
    ...
    async fn get_load(
        ...
        self.inner
            .clone()
            .get_load(tonic_request)
            .await
            .map(|response| response.into_inner())
            .map_err(|status| crate::error::grpc_status_to_service_error(
                status,
                GetLoadRequest::rpc_name(),
            ))
    }
}
#[derive(Debug)]
...
impl search_service_grpc_server::SearchServiceGrpc for SearchServiceGrpcServerAdapter {
    ...
    async fn get_load(
        &self,
        tonic_request: tonic::Request<GetLoadRequest>,
    ) -> Result<tonic::Response<GetLoadResponse>, tonic::Status> {
        let parent_context = quickwit_common::tracing_utils::extract_context(
            tonic_request.metadata(),
        );
        let request = tonic_request.into_inner();
        let span = tracing::info_span!("search.get_load");
        let _ = <tracing::Span as tracing_opentelemetry::OpenTelemetrySpanExt>::set_parent(
            &span,
            parent_context,
        );
...
pub mod search_service_grpc_server {
    ...
    #[async_trait]
    pub trait SearchServiceGrpc: std::marker::Send + std::marker::Sync + 'static {
        /// Root search API.
        /// This RPC identifies the set of splits on which the query should run on,
        /// and dispatch the several calls to `LeafSearch`.
        ///
    ...
    #[derive(Debug)]
    pub struct SearchServiceGrpcServer<T> {
        inner: Arc<T>,
    ...
    impl<T> SearchServiceGrpcServer<T> {
        pub fn new(inner: T) -> Self {
        ...
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/quickwit.search.SearchService/RootSearch" => {
                    ...
                    for RootSearchSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as SearchServiceGrpc>::root_search(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/LeafSearch" => {
                    ...
                    for LeafSearchSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::leaf_search(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/FetchDocs" => {
                    ...
                    for FetchDocsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::fetch_docs(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/RootListTerms" => {
                    ...
                    for RootListTermsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::root_list_terms(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
                ...
                "/quickwit.search.SearchService/LeafListTerms" => {
                    ...
                    for LeafListTermsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::leaf_list_terms(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
                ...
                "/quickwit.search.SearchService/Scroll" => {
                    ...
                    for ScrollSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::scroll(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/PutKV" => {
                    ...
                    > tonic::server::UnaryService<super::PutKvRequest> for PutKVSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::put_kv(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/GetKV" => {
                    ...
                    > tonic::server::UnaryService<super::GetKvRequest> for GetKVSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::get_kv(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/ReportSplits" => {
                    ...
                    for ReportSplitsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::report_splits(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
                ...
                "/quickwit.search.SearchService/ListFields" => {
                    ...
                    for ListFieldsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::list_fields(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/LeafListFields" => {
                    ...
                    for LeafListFieldsSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::leaf_list_fields(&inner, request)
                                    .await
                            };
                            Box::pin(fut)
                        }
                ...
                "/quickwit.search.SearchService/SearchPlan" => {
                    ...
                    for SearchPlanSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::search_plan(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/quickwit.search.SearchService/GetLoad" => {
                    #[allow(non_camel_case_types)]
                    struct GetLoadSvc<T: SearchServiceGrpc>(pub Arc<T>);
                    impl<
                        T: SearchServiceGrpc,
                    ...
                    for GetLoadSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as SearchServiceGrpc>::get_load(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for SearchServiceGrpcServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T> tonic::server::NamedService for SearchServiceGrpcServer<T> {
