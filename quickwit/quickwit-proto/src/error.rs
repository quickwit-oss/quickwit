...
where E: GrpcServiceError {
    ...
    match encode_error(&service_error) {
        ...
        Err(error) => {
            warn!(%error, "failed to encode service error `{service_error:?}`");
        }
    }
    status
}
...
where E: GrpcServiceError {
    if let Some(header_value) = status.metadata().get_bin(QW_ERROR_HEADER_NAME) {
        let service_error = match decode_error(header_value) {
            ...
            Err(error) => {
                let message = format!(
                    "failed to deserialize service error returned from server (this can happen \
                     during rolling upgrades): {error}"
                );
...
fn encode_error<E: Serialize>(service_error: &E) -> anyhow::Result<BinaryMetadataValue> {
    ...
    let service_error_bytes = Bytes::from(service_error_json);
    let header_value = BinaryMetadataValue::try_from(service_error_bytes)?;
    Ok(header_value)
}
