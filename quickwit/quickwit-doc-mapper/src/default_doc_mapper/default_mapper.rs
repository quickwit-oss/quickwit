...
use crate::{
    index_tokenizer_manager, DocMapper, DocParsingError, ModeType, QueryParserError, WarmupInfo,
...
pub struct DefaultDocMapper {
    ...
    schema: Schema,
    // TokenizerManager generated from the field mappings parameters.
...
impl TryFrom<DefaultDocMapperBuilder> for DefaultDocMapper {
    ...
    fn try_from(builder: DefaultDocMapperBuilder) -> anyhow::Result<DefaultDocMapper> {
        ...
        let mut regex_tokenizer_patterns: HashSet<String> = HashSet::new();
        let mut schema_builder = Schema::builder();
        let field_mappings = build_mapping_tree(
            &builder.field_mappings,
            &mut schema_builder,
            &mut regex_tokenizer_patterns,
        )?;
        ...
        let tokenizer_manager = index_tokenizer_manager(regex_tokenizer_patterns);
...
mod tests {
    ...
    #[test]
    fn test_default_doc_mapper_collects_regex_patterns() {
        let default_doc_mapper: DefaultDocMapper = serde_json::from_str(
            r#"{
            "field_mappings": [
                {
        ...
        assert!(default_doc_mapper
            ...
            .get("[0-9]+")
            .is_some());
        assert!(default_doc_mapper.tokenizer_manager().get("\\W+").is_some());
        assert!(default_doc_mapper
            .tokenizer_manager()
            .get("[a-z]{5}")
            .is_some());
