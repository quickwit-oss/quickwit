...
pub struct DefaultDocMapper {
    ...
    concate_dynamic_fields: Vec<Field>,
...
impl TryFrom<DefaultDocMapperBuilder> for DefaultDocMapper {
    ...
    fn try_from(builder: DefaultDocMapperBuilder) -> anyhow::Result<DefaultDocMapper> {
        ...
        let (field_mappings, concate_dynamic_fields) =
            build_mapping_tree(&builder.field_mappings, &mut schema_builder)?;
        if !concate_dynamic_fields.is_empty() && dynamic_field.is_none() {
            bail!("concatenate field uses _dynamic, but index isn't dynamic");
        }
    ...
    fn doc_from_json_obj(
        ...
        if let Some(dynamic_field) = self.dynamic_field {
            if !dynamic_json_obj.is_empty() {
                if !self.concate_dynamic_fields.is_empty() {
                    let json_obj_values = dynamic_json_obj
                        .iter()
                        .flat_map(|(_key, val)| JsonValueIterator::new(val.clone()))
