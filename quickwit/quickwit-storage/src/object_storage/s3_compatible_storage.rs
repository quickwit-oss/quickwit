...
impl S3CompatibleObjectStorage {
    ...
    async fn put_single_part_single_try<'a>(
        ...
        crate::OBJECT_STORAGE_PUT_PARTS.increment(1);
        crate::OBJECT_STORAGE_UPLOAD_NUM_BYTES.increment(len);
    ...
    async fn upload_part<'a>(
        ...
        crate::OBJECT_STORAGE_PUT_PARTS.increment(1);
        crate::OBJECT_STORAGE_UPLOAD_NUM_BYTES.increment(part.len());
    ...
    async fn get_object(
        ...
        crate::OBJECT_STORAGE_GET_TOTAL.increment(1);
...
async fn download_all(byte_stream: ByteStream) -> StorageResult<Bytes> {
    ...
    crate::OBJECT_STORAGE_DOWNLOAD_NUM_BYTES.increment(bytes.len() as u64);
    Ok(bytes)
}
...
impl Storage for S3CompatibleObjectStorage {
    ...
    async fn put(
        ...
        crate::OBJECT_STORAGE_PUT_TOTAL.increment(1);
        let _permit = REQUEST_SEMAPHORE.acquire().await;
        let key = self.key(path);
    ...
    async fn copy_to(&self, path: &Path, output: &mut dyn SendableAsync) -> StorageResult<()> {
        ...
        crate::OBJECT_STORAGE_DOWNLOAD_NUM_BYTES.increment(num_bytes_copied);
        output.flush().await?;
        Ok(())
    }
