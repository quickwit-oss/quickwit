...
impl MultiPartPolicy {
    ...
    pub fn part_num_bytes(&self, len: u64) -> u64 {
        assert!(
            len < self.max_object_num_bytes.as_u64(),
            "This object storage does not support object of that size {}",
            self.max_object_num_bytes.as_u64()
        );
        assert!(
            self.max_num_parts > 0,
            "Misconfiguration: max_num_parts == 0 makes no sense."
        );
        if len < self.multipart_threshold_num_bytes.as_u64() || self.max_num_parts == 1 {
            return len;
        }
        ...
        min_part_len.max(self.target_part_num_bytes.as_u64())
    }
...
impl Default for MultiPartPolicy {
    fn default() -> Self {
        MultiPartPolicy {
            ...
            target_part_num_bytes: ByteSize::gib(5),
            multipart_threshold_num_bytes: ByteSize::mib(128),
            max_num_parts: 10_000,
            max_object_num_bytes: ByteSize::tb(5), // S3 allows up to 5TB objects
