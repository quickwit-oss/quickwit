...
mod tests {
    ...
    fn sorted_split_ulids(num_splits: usize) -> Vec<Ulid> {
        let mut split_ulids: Vec<Ulid> = std::iter::repeat_with(Ulid::new).take(num_splits).collect();
        split_ulids.sort();
        split_ulids
    }
    ...
    #[test]
    fn test_split_table() {
        let mut split_table = SplitTable::with_limits_and_existing_splits(SplitCacheLimits {
