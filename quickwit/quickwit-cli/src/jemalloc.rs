...
use quickwit_common::metrics::{
    DD_MEMORY_ALLOCATED_BYTES, MEMORY_ACTIVE_BYTES, MEMORY_ALLOCATED_BYTES, MEMORY_RESIDENT_BYTES,
};
...
pub async fn jemalloc_metrics_loop() -> tikv_jemalloc_ctl::Result<()> {
    ...
    loop {
        ...
        match allocated_mib.read() {
            Ok(allocated) => {
                MEMORY_ALLOCATED_BYTES.set(allocated as f64);
                DD_MEMORY_ALLOCATED_BYTES.set(allocated as f64);
            }
            Err(error) => {
