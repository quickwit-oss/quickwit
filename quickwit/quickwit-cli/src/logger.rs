...
fn parse_otlp_protocol(protocol_str: &str) -> anyhow::Result<OtlpProtocol> {
    ...
    match protocol_str {
        ...
        other => anyhow::bail!(
            "unsupported OTLP protocol `{other}`, supported values are `{OTLP_PROTOCOL_GRPC}`, `{OTLP_PROTOCOL_HTTP_PROTOBUF}` and `{OTLP_PROTOCOL_HTTP_JSON}`"
        ),
    }
}
...
pub fn setup_logging_and_tracing(
    ...
    let provider_opt = if get_bool_from_env(QW_ENABLE_OPENTELEMETRY_OTLP_EXPORTER_ENV_KEY, false) {
        ...
        let span_exporter = build_otlp_exporter!(
            opentelemetry_otlp::SpanExporter::builder(),
            traces_protocol
        )
        .context(format!("failed to initialize OTLP traces exporter"))?;
        ...
        let logs_exporter = build_otlp_exporter!(
            opentelemetry_otlp::LogExporter::builder(),
            logs_protocol
        )
        .context(format!("failed to initialize OTLP logs exporter"))?;
