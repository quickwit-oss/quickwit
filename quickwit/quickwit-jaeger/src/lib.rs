...
mod tests {
    ...
    fn test_build_query() {
        ...
        }
        {
            ...
            assert_eq!(
                build_query(service_name, span_kind, span_name, tags),
                "service_name:quickwit AND span_kind.name:client AND span_attributes.baz:qux AND \
                 span_attributes.foo:bar"
            );
        }
        {
            ...
            assert_eq!(
                build_query(service_name, span_kind, span_name, tags),
                "service_name:quickwit AND span_kind.name:client AND span_name:leaf_search AND \
                 span_attributes.baz:qux AND span_attributes.foo:bar"
            );
        }
    }
