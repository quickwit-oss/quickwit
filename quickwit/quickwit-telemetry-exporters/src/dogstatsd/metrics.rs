...
use quickwit_common::{get_from_env, get_from_env_opt};
...
pub(crate) fn build_recorder(service_version: &str) -> anyhow::Result<DogStatsDRecorder> {
    ...
    let host: String =
        get_from_env_opt("CLOUDPREM_DOGSTATSD_SERVER_HOST", false).unwrap_or_else(|| {
            get_from_env("CP_DOGSTATSD_SERVER_HOST", "127.0.0.1".to_string(), false)
        });
    let port: u16 = get_from_env_opt("CLOUDPREM_DOGSTATSD_SERVER_PORT", false)
        .unwrap_or_else(|| get_from_env("CP_DOGSTATSD_SERVER_PORT", 8125, false));
    ...
    for (env_var_key, label_key) in keys {
        if let Some(label_val) = get_from_env_opt::<String>(env_var_key, false) {
