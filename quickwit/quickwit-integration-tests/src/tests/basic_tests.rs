...
#[tokio::test]
async fn test_merge_pipelines_fallback_when_compaction_enabled_but_no_janitor() {
    quickwit_common::setup_logging_for_tests();
    unsafe { std::env::set_var("QW_ENABLE_COMPACTION_SERVICE", "true") };

    // No janitor node — indexer should fail to find the compaction service
    // and fall back to local merge pipelines.
    let sandbox = ClusterSandboxBuilder::default()
        .add_node([QuickwitService::Searcher])
        .add_node([QuickwitService::Metastore])
        .add_node([QuickwitService::Indexer])
