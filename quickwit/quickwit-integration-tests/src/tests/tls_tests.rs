...
async fn test_tls_rest_max_connection_age() {
    ...
    let connection_opened_at = Instant::now();
    ...
    // Guard against the test passing for the wrong reason: the connection must be closed *because*
    // of the 2s max age, not because idle/unused connections are reaped quickly. We allow some
    // slack below 2s since the server's age timer starts at accept time, before the handshake we
    // measure from completes.
    let connection_lifetime = connection_opened_at.elapsed();
    assert!(
        connection_lifetime >= Duration::from_millis(1500),
        "connection closed after {connection_lifetime:?}, expected it to stay open close to the \
         2s max connection age"
    );
