...
| `shutdown_drain_timeout` | Time budget granted to each indexing pipeline to drain gracefully before giving up, on node shutdown and when the control plane tears down a pipeline. It must exceed the largest `commit_timeout_secs` plus the time to upload and publish the final splits, and the deployment's shutdown grace period (e.g. `terminationGracePeriodSeconds` on Kubernetes) must exceed it in turn. Can be overridden with the `QW_INDEXER_SHUTDOWN_DRAIN_TIMEOUT` environment variable. | `300s` |
