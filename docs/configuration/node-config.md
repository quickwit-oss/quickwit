...
| `partial_request_cache_capacity` | Partial request in memory cache capacity on a Searcher. Cache intermediate state for a request, possibly making subsequent requests faster. Entries are keyed by a 128-bit hash of the request, with a negligeable probability of cache collision. It can be disabled by setting the size to `0`. | `64M` |
