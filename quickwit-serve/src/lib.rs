...
pub async fn serve_quickwit(
    ...
    let indexer_service: Option<Mailbox<IndexingService>> =
        if services.contains(&QuickwitService::Indexer) {
            // In a single mode deployement, the indexer is running alongside the control plane.
            // If present, it's better to directly use the available local metastore service.
            // If not, let's use the metastore service client.
