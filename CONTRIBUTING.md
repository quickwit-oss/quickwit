...
In configuration and API tables, use `null` for absent optional defaults. Use actual values
such as `[]`, `{}`, or `false` for empty collections and disabled boolean options, and preserve
literal enum values such as `none`.
