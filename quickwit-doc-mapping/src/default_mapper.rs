...
use tantivy::{Document, query::Query, schema::{DocParsingError, FieldEntry, FieldValue, STORED, Schema, SchemaBuilder}};
...
impl DefaultDocMapper {
    ...
    fn fill_document(&self, json_obj: &JsonValue, document: &mut Document) -> Result<(), DocParsingError> {
...
mod tests {
    use std::{collections::HashMap};
    use serde_json::{self, Value as JsonValue};
    use super::{DocMapper, DocMapperConfig, DefaultDocMapper, get_json_paths_and_values};
    ...
    fn test_get_json_paths_and_values() {
        ...
        let expected_json_paths_and_values:HashMap<String, JsonValue>  = serde_json::from_str(EXPECTED_JSON_PATHS_AND_VALUES).unwrap();
        let mut values_by_json_path = HashMap::new();
        let mut json_path: Vec<String> = vec![];
        get_json_paths_and_values(&JsonValue::Object(json_object), &mut json_path, &mut values_by_json_path);
        for (json_path, values) in values_by_json_path.iter() {
            assert_eq!(expected_json_paths_and_values.contains_key(json_path), true);
            let expected_values = expected_json_paths_and_values.get(json_path).unwrap().as_array().unwrap();
